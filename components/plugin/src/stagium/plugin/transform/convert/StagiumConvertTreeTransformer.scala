...
trait StagiumConvertTreeTransformer {
  ...
  //   03) We also assume that users can't define custom getters for stagium fields => these getters can be regarded as pure
  ...
  //   06) TODO: support stagium fields of stagium class types
  ...
  // C => any class
  // V => any stagium class
  // VS, VM => single-field stagium class and multi-field stagium class
  // u => method that has one or more of its parameters with type V @unboxed
  // r => method that returns V @unboxed
  // T => any type parameter
  // x, y => underlying fields or getters of the stagium class with types X and Y
  // f => any selector
