  ...
  abstract class TreeRewriter(unit: CompilationUnit) extends TypingTransformer(unit) {
    ...
    override def transform(tree: Tree): Tree = {
      ...
      def commit(rule: Option[String], tree1: Result): Result = {
        ...
        def logRewrite() = stagiumlog(s"$rulePrefix$treeString -> $tree1")
        def logRetype() = stagiumlog(s"$rulePrefix$treeString: $treeTpeString -> ${tree1.asInstanceOf[Single].tree.tpe}")
      ...
      stats flatMap {
        case stat =>
          ...
          def commit(rule: Option[String], stats1: Result): Result = {
            val rulePrefix = rule.map(rule => s"$rule) ").getOrElse("")
            def logRewrite() = stagiumlog(s"$rulePrefix$statString -> $stats1")
            def logRetype() = stagiumlog(s"$rulePrefix$statString: $statTpeString -> ${stats1.asInstanceOf[Single].tree.tpe}")
    ...
    case class State(tree: Tree, owner: Symbol, commit: (Option[String], Result) => Result, fallback: () => Result) {
      ...
      def explode(v: Symbol, x: Symbol, tpe: Type, rhs: Tree): ValDef = {
        ...
        stagiumlog(s"VALDEF: $v -> ${exploded.symbol}")
      ...
      def explode(p: Symbol, x: Symbol): Symbol = {
        p.explodedSymbols.get(x.name) getOrElse {
          ...
          stagiumlog(s"PARAMDEF: $p -> $exploded")
          exploded
        }
      }
